...
def effective_rank(eigenv):
    try:
        assert eigenv.min() > 0, "eigenvalues must be positive"
    except AssertionError as e:
        print(e)
        print(eigenv.min())
    norm_eigenv = eigenv / eigenv.sum(1).unsqueeze(1)  # [B, N]
    entropy = -torch.sum((norm_eigenv * torch.log(norm_eigenv)), dim=1)
...
def get_cosine_lr_func(lr_init, lr_final, lr_delay_steps=0, lr_delay_mult=1.0, max_steps=1000000):
    def helper(step):
        if step < 0 or (lr_init == 0.0 and lr_final == 0.0):
            # Disable this parameter
            return 0.0
        if lr_delay_steps > 0:
            ...
            delay_rate = 1.0
        t = np.clip(step / max_steps, 0, 1)
        cosine_lerp = np.cos(t * np.pi / 2) * (lr_init - lr_final) + lr_final
        return delay_rate * cosine_lerp
