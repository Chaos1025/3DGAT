...
class GaussianModel:
    def setup_functions(self, scale_bound):
        ...
        self.scaling_activation = lambda x: torch.sigmoid(x) * scale_max_bound + scale_min_bound
        ...
        #     lambda x: torch.sigmoid(x) * 100
        # )
        # self.density_inverse_activation = lambda x: inverse_sigmoid(
        #     torch.relu(x / 100) + 1e-8
        # )
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale: float, init_from="pcd"):
        # Create gaussian model from initialized point cloud
        self.spatial_lr_scale = spatial_lr_scale
        # print(init_from)
        if init_from in ["pcd", "random", "unifrm", "measur"]:
            # Initialize gaussians density and position
            if init_from == "pcd":
                ...
                #     .cuda()
                # )
                fused_density = torch.tensor(np.asarray(pcd.density)).float().cuda()
                # fused_density = torch.rand(np.asarray(pcd.density).shape).float().cuda() * 0.1
            elif init_from == "measur":
                fused_point_cloud = pcd[:, :-1].float().cuda()
                fused_density = self.density_inverse_activation(pcd[:, -1:].float().cuda())
                print(
                    "Initialize gaussians from {} estimated points".format(
                        fused_point_cloud.shape[0]
                    )
                ...
                fused_density = self.density_inverse_activation(pcd[:, -1:].float().cuda())
                print(
                    "Initialize gaussians from {} random points".format(fused_point_cloud.shape[0])
                )
                ...
                fused_density = self.density_inverse_activation(pcd[:, -1:].float().cuda())
                print(
                    "Initialize gaussians from {} uniform points".format(fused_point_cloud.shape[0])
                )

            try:
                scales = torch.tensor(np.asarray(pcd.scales)).float().cuda()
                rots = torch.tensor(np.asarray(pcd.rots)).float().cuda()
                print(f"Scales shape:{scales.shape}, Rotations shape:{rots.shape} ")
            except:
                print("WARNING: scales and rotations not on point cloud, random initialization")
            ...
            scales = self.scaling_inverse_activation(torch.tensor(pcd["scale"]).float().cuda())
            rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
            rots[:, 0] = 1
            fused_density = self.density_inverse_activation(
    ...
    def load_ply(self, path):
        ...
        self._density = nn.Parameter(
            torch.tensor(densities, dtype=torch.float, device="cuda").requires_grad_(True)
        )
    ...
    def densify_and_split(self, grads, grad_threshold, densify_scale_threshold, N=2):
        ...
        # ** OVER RECONSTRUCTION
        # gradient norm is large enough and scale is big
        ...
        if new_xyz.shape[0] != 0:
            print("Split {} points".format(new_xyz.shape[0] // N))
    ...
    def densify_and_clone(self, grads, grad_threshold, densify_scale_threshold):
        ...
        # gradient norm is large enough and scale is small
        ...
        if new_xyz.shape[0] != 0:
            print("Clone {} points".format(new_xyz.shape[0]))
    ...
    def densify_and_prune(
        ...
        print("\nMax gradient norm:", torch.max(torch.norm(grads, dim=-1)).item())
        ...
        if prune_mask.shape[0] != 0:
            print("Prune {} points".format(torch.sum(prune_mask).item()))
